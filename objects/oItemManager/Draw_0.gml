...
for ( var i = 0; i < array_length( inv ); i++ )
	...
	draw_text ( camera_get_view_x( view_camera[0] ) + 16, camera_get_view_y( view_camera[0] ) + _sep*i, inv[i].name );
	
	//Description text of said item.
	if selected_item == i
		{
		draw_text_ext( _xx, _yy + _sep*array_length( inv ), inv[i].description, 12, 100 );
		//draw_text_ext( camera_get_view_x( view_camera[0] ), _yy + _sep*array_length( inv ), inv[i].description, 12, 100 );
		}
